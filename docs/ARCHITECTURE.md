...
- Bright Chalk (`--font-special`) for special elements, currently the identity colour word and the sticky values voice.
...
- A three-viewport sticky section maps ordinary scroll progress to the eight frames through a velocity-bounded visual follower. Slow input stays close to native progress, while strong wheel and trackpad input catches up at a capped rate so intermediate frames remain visible.
