...
- Destination routes now use a shared three-column field composition on desktop, with the paper in the center and the Home control held in the exposed left rail. Mobile recomposes this into a reserved Home band above the paper.
- The destination Home control uses Igor's `Doodles_home.png` drawing as its visible form while retaining the semantic link and accessible name `Home`. The full-resolution PNG is downscaled in layout so its fine strokes remain crisp.
- `PaperSurface` has an explicit notebook variant that combines the warm paper token with the retained crumpled-paper material and restrained grain. A shared irregular polygon edge and separate drop shadow keep destination sheets visibly imperfect without changing their content layout.
...
- Each craft-value box is a native checkbox with a 44-pixel target, visible keyboard focus, varied hand-drawn geometry, and a compact two-stroke checking gesture that reverses cleanly when unchecked. Checking an item draws one loose strikethrough without layering a second browser decoration over it. Completing all three lets the final check settle before scrolling only as far as needed to the invitation `I'd love to chat with you.` and the same canonical Connect options used on Home.
